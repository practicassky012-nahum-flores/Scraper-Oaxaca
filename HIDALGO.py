...
# Iniciar el navegador
driver.get('http://compilacion.ordenjuridico.gob.mx/poderes2.php?edo=13')
...
download_directory = "C:\\Users\\pro02\\Documents\\azurite\\HIDALGO DOF"
...
try:
    ...
    # Lista de IDs de archivos problemáticos a omitir
    archivos_omitir = ['79108']  # Puedes agregar más IDs aquí si encuentras otros archivos problemáticos
    ...
    # Variable para rastrear si ya encontramos el archivo problemático
    archivo_problematico_encontrado = False
    ...
    for fila in filas:
        ...
        # Extraer el idArchivo del enlace para verificar si debe omitirse
        href = enlace.get_attribute('href')
        id_archivo = None
        if 'idArchivo=' in href:
            try:
                id_archivo = href.split('idArchivo=')[1].split('&')[0]
            except:
                pass

        # Verificar si este archivo está en la lista de archivos a omitir
        if id_archivo in archivos_omitir:
            logging.warning(f"Omitiendo archivo problemático con ID {id_archivo}: {nombre_ordenamiento}")
            archivo_problematico_encontrado = True
            continue  # Saltar este archivo y continuar con el siguiente

        # Si aún no hemos encontrado el archivo problemático, omitir todos los archivos anteriores
        if not archivo_problematico_encontrado:
            logging.info(f"Omitiendo archivo anterior al problemático: {nombre_ordenamiento}")
            continue
