...
def _clasificar_materia_nueva(titulo: str) -> list:
    """Clasifica materia con 37+ categorías (usado en formato nuevo de 12 campos)."""
    t = titulo.lower()
    materias = []
    if 'constitución' in t or 'constitucional' in t: materias.append('constitucional')
    if 'código civil' in t or ('civil' in t and 'código' in t): materias.append('civil')
    elif 'civil' in t and 'protección civil' not in t: materias.append('civil')
    if 'penal' in t or 'delito' in t or 'ejecución de penas' in t: materias.append('penal')
    if any(x in t for x in ['fiscal', 'impuesto', 'tributar', 'hacienda', 'ingreso', 'egreso', 'presupuesto', 'deuda pública']): materias.append('fiscal')
    if any(x in t for x in ['electoral', 'elección', 'partido', 'voto']): materias.append('electoral')
    if any(x in t for x in ['trabajo', 'trabajador', 'laboral', 'salario']): materias.append('laboral')
    if any(x in t for x in ['transparencia', 'acceso a la información', 'datos personales']): materias.append('transparencia')
    if any(x in t for x in ['derechos humanos', 'víctima', 'discriminación']): materias.append('derechos humanos')
    if any(x in t for x in ['género', 'mujeres', 'violencia contra', 'igualdad entre']): materias.append('género')
    if any(x in t for x in ['niñas', 'niños', 'adolescentes', 'menores']): materias.append('derechos de la niñez')
    if any(x in t for x in ['salud', 'médic', 'hospital', 'sanitari']): materias.append('salud')
    if any(x in t for x in ['educación', 'educativ', 'escuela', 'universidad']): materias.append('educación')
    if any(x in t for x in ['cultura', 'patrimonio cultural', 'arte', 'museo']): materias.append('cultura')
    if any(x in t for x in ['desarrollo social', 'asistencia social', 'pobreza', 'bienestar']): materias.append('desarrollo social')
    if any(x in t for x in ['urbano', 'territorial', 'asentamientos humanos']): materias.append('urbanismo')
    if any(x in t for x in ['ambiente', 'ecológ', 'ambiental', 'forestal', 'cambio climático', 'sustentable']): materias.append('ambiental')
    if 'protección civil' in t or 'emergencia' in t: materias.append('protección civil')
    if any(x in t for x in ['seguridad pública', 'policía', 'guardia']): materias.append('seguridad pública')
    if any(x in t for x in ['ciencia', 'tecnología', 'innovación']): materias.append('ciencia y tecnología')
    if any(x in t for x in ['deporte', 'cultura física']): materias.append('deporte')
    if any(x in t for x in ['municipal', 'municipio', 'ayuntamiento']): materias.append('municipal')
    if 'turismo' in t or 'turístic' in t: materias.append('turismo')
    if any(x in t for x in ['transporte', 'tránsito', 'vialidad', 'camino']): materias.append('movilidad')
    if any(x in t for x in ['agrari', 'rural', 'campo', 'ejid', 'agríco']): materias.append('agrario')
    if any(x in t for x in ['económico', 'competencia', 'comercio', 'inversión', 'industria']): materias.append('económico')
    if any(x in t for x in ['energía', 'eléctric', 'petróleo']): materias.append('energía')
    if any(x in t for x in ['anticorrupción', 'contraloría', 'responsabilidad']): materias.append('anticorrupción')
    if 'notari' in t or 'fe pública' in t: materias.append('notariado')
    if 'registro' in t and 'registro civil' not in t: materias.append('registral')
    if any(x in t for x in ['migra', 'refugiado', 'asilo']): materias.append('migratorio')
    if 'ética' in t or 'conducta' in t: materias.append('ética')
    if any(x in t for x in ['congreso', 'cámara', 'senado', 'legislat', 'parlamentar']): materias.append('parlamentario')
    if any(x in t for x in ['auditoría', 'fiscalización', 'cuenta pública']): materias.append('auditoría')
    if any(x in t for x in ['tribunal', 'judicial', 'judicatura', 'justicia']): materias.append('judicial')
    if 'participación ciudadana' in t or 'consulta popular' in t: materias.append('participación ciudadana')
    if any(x in t for x in ['seguro', 'fianza', 'crédito', 'bancari', 'financier', 'valores']): materias.append('financiero')
    if any(x in t for x in ['militar', 'ejército', 'armada']): materias.append('militar')
    if any(x in t for x in ['agua', 'hídric', 'acuícol', 'pesca']): materias.append('agua y pesca')
    if any(x in t for x in ['vivienda', 'habitacional']): materias.append('vivienda')
    if any(x in t for x in ['organ', 'administración pública']) and 'administrativo' not in materias: materias.append('administrativo')
    if any(x in t for x in ['indígena', 'pueblo', 'comunidad indígena']): materias.append('pueblos indígenas')
    return materias if materias else ['general']


def _detectar_tipo_ordenamiento(titulo: str) -> str:
    """Detecta tipo de ordenamiento para el formato nuevo."""
    t = titulo.lower()
    if 'constitución' in t: return 'Constitución'
    if 'código' in t: return 'Código'
    if 'reglamento' in t: return 'Reglamento'
    if 'decreto' in t: return 'Decreto'
    if 'estatuto' in t: return 'Estatuto'
    if 'ordenanza' in t: return 'Ordenanza'
    if 'presupuesto' in t: return 'Presupuesto'
    if 'acuerdo' in t: return 'Acuerdo'
    if 'norma' in t or 'nom-' in t: return 'NOM'
    if 'tratado' in t or 'convención' in t: return 'Convención'
    if 'ley' in t: return 'Ley'
    return 'Otro'


def _fecha_a_iso(fecha_str: str):
    """Convierte DD/MM/YYYY → YYYY-MM-DD. Retorna None si no se puede."""
    if not fecha_str:
        return None
...
def guardar_json_texto(doc: dict, nombre_seccion: str,
                       ...
                       json_path: Path) -> None:
    # Título: para Federal usa tabla HTML, para las demás usa OCR si hay
    if nombre_seccion == "Federal":
        titulo_final = doc["titulo"]
    else:
    ...
    # Fecha a ISO
    fecha_pub = _fecha_a_iso(doc.get("fecha_pub", ""))
    if not fecha_pub:
        fecha_pub = _fecha_a_iso(doc.get("fecha_reforma", ""))
...
def _extraer_y_guardar(pdf_path: Path, json_path: Path,
    ...
    # Renombrar JSON y PDF con el título correcto
    if nombre_seccion == "Federal":
        titulo_final = doc["titulo"]
    else:
        titulo_final = titulo_real if titulo_real else doc["titulo"]
