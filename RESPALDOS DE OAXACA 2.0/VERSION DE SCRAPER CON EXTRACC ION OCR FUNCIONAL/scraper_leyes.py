...
║     + OCR (pdfplumber / Tesseract) + JSON enriquecido              ║
╚══════════════════════════════════════════════════════════════════════╝
...
def _procesar_pdf_completo(pdf_path: Path) -> dict:
    ...
    try:
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            metodo = "pdfplumber" if not pags_ocr else f"pdfplumber+tesseract ({pags_ocr} págs OCR)"
...
def procesar_textos_seccion(nombre: str, docs: list, carpeta_pdfs: Path) -> None:
    ...
    for d in docs:
        ...
        pdf_path  = carpeta_pdfs / f"{base}.pdf"
...
def main():
    ...
    if _TESSERACT_OK:
        print("  ✓ Tesseract disponible (título desde imagen + OCR escaneados)")
    else:
        print("  ℹ Tesseract no disponible — título vía pdfplumber + correcciones")
